...
class AddNotebookViewController: UIViewController {
    ...
    lazy var textField: UITextField = {
        ...
        textField.placeholder = NSLocalizedString("Insert notebook title and tap Submit", comment: "")

        return textField
    }()
        
    let viewModel: AddNotebookViewModel

    init(viewModel: AddNotebookViewModel) {
    ...
    fileprivate func showErrorAddingNotebookAlert() {
        let message = NSLocalizedString("Error adding notebook", comment: "")
        showAlert(message)
    }
    
}

extension AddNotebookViewController: AddNotebookViewDelegate {
    func errorAddingNotebook() {
        showErrorAddingNotebookAlert()
    }
}
